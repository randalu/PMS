...
## 2026-05-16 - Prevent N+1 on lockForUpdate loops using whereIn
**Learning:** Calling `$item->variant()->lockForUpdate()->first()` inside a loop over `$items` creates an N+1 performance and locking bottleneck during critical transactional flows like order confirmation.
**Action:** Pluck all unique related IDs upfront, execute a single `ProductVariant::whereIn('id', $ids)->lockForUpdate()->get()->keyBy('id')` query, and use O(1) hash map lookups (`$variants->get(...)`) inside the loop.
