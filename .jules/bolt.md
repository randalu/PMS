...
## 2026-06-24 - Prevent N+1 query when locking models in a loop
**Learning:** Sequential database locking (e.g. `->lockForUpdate()->first()` inside a `foreach`) causes an N+1 query performance bottleneck and theoretically increases the chance of deadlocks.
**Action:** Batch fetch all required IDs with `pluck()`, and lock them outside the loop using a single `whereIn()->lockForUpdate()->get()->keyBy('id')` query. Then fetch from the collection inside the loop.
